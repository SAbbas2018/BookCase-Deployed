...
/*
    ReqType: POST URL: /home/library/add
    Access: Public
    Desc: add new book to user library list in db
*/
...
/*
    ReqType: POST URL: /home/library/getBook
    Access: Public
...
router.post("/getBook", async (req, res) => {
  try {
    ...
    let book = { bookTitle, bookAuthor, bookImg };
...
router.post("/deleteBook", async (req, res) => {
  try {
    ...
    await User.updateOne(
      { email: email },
      { $pull: { library: { bookTitle: bookTitle } } }
    );
    res.status(200).json({ message: "success" });
  } catch (err) {
    res.status(500).json({ error: err.message });
